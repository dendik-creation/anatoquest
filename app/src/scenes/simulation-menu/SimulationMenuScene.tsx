...
export function SimulationMenuScene({ onBackToHome, onSelectSimulation }: Props) {
  ...
  return <main className="simulation-menu" data-testid="simulation-menu-scene" style={{ '--stage-scale': scale } as CSSProperties}><div className="simulation-menu__stage">
    ...
    <button className="simulation-menu__icon simulation-menu__audio" type="button" aria-label={audioOn ? 'Matikan musik latar' : 'Aktifkan musik latar'} aria-pressed={audioOn} onClick={() => setAudioOn((value) => !value)}><img src={audioOn ? bgmOnArt : bgmOffArt} alt="" /></button><button className="simulation-menu__icon simulation-menu__help" type="button" aria-label="Bantuan simulasi organ"><img src={helpArt} alt="" /></button>
