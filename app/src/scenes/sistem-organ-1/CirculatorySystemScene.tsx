...
export function CirculatorySystemScene({ onBackToHome, onBack, onComplete, transitionState = 'entered' }: CirculatorySystemSceneProps) {
  ...
  return <main className="circulatory-system" data-testid="circulatory-system-scene" data-microscene="5.5" data-transition={transitionState} data-selected={selected} style={style} aria-labelledby="circulatory-system-heading">
    <div className="circulatory-system__stage">
      ...
      <button className="circulatory-system__icon circulatory-system__top-back" type="button" aria-label="Kembali ke materi sebelumnya" onClick={onBack}><img src={backArt} alt="" /></button>
      <button className="circulatory-system__icon circulatory-system__audio" type="button" aria-label={audioOn ? 'Matikan musik latar' : 'Aktifkan musik latar'} aria-pressed={audioOn} onClick={() => setAudioOn((value) => !value)}><img src={audioOn ? bgmOnArt : bgmOffArt} alt="" /></button>
      <HelpButton className="circulatory-system__icon circulatory-system__help" label="Bantuan sistem jantung dan pembuluh darah" onClick={() => setShowHint((value) => !value)} />
      ...
      {showHint && <p className="circulatory-system__hint" role="status">Pilih Jantung, Arteri, Vena, atau Kapiler untuk menyorot bagian tersebut dan membaca informasinya.</p>}
