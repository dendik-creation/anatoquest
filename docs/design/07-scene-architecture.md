...
| SC-03 | ~~Petunjuk Penggunaan~~ | Superseded (`DD-14`) | Not a scene. Every scene ships a reusable help/`?` icon button that runs a per-scene driver.js-style guided-tour overlay instead. | — | — | React (shared button + overlay component, per-scene step content) | — |
